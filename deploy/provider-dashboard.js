...
async function init() {
    try {
        ...
        const requiredElements = {
            'dashboard-facilities': dashboardFacilities,
        ...
        if (storedProvider) {
            try {
                currentProvider = JSON.parse(storedProvider);
                ...
                currentProvider.id = currentProvider.id || 0;
                currentProvider.firstName = currentProvider.firstName || 'Demo';
                currentProvider.lastName = currentProvider.lastName || 'Provider';
                currentProvider.email = currentProvider.email || 'demo@demo.com';
                currentProvider.facilities = Array.isArray(currentProvider.facilities)
                    ? currentProvider.facilities
                    : [];

            } catch (parseError) {
                console.error('Failed to parse provider data:', parseError);
                // Clear corrupted data and create demo provider
                localStorage.removeItem('currentProvider');
                currentProvider = createDemoProvider();
            }
        } else {
            // Create default demo provider if none exists
            currentProvider = createDemoProvider();
        }

        // Save validated provider data
        try {
            localStorage.setItem('currentProvider', JSON.stringify(currentProvider));
        } catch (storageError) {
            console.error('Failed to save provider data:', storageError);
        }
        ...
        if (dashboardFacilities) {
            dashboardFacilities.innerHTML = `
                <div class="error-state">
                    <h3>Dashboard Error</h3>
                    <p>There was a problem loading the dashboard. Please refresh the page.</p>
...
// Create a demo provider with default values
...
function handleFacilitySearch() {
    ...
    availableFacilities.forEach(f => {
        ...
        nameDiv.textContent = safeFacility ? safeFacility.name : (f.name || '');

        const addressDiv = document.createElement('div');
        addressDiv.className = 'search-item-address';
        addressDiv.textContent = safeFacility ? safeFacility.address : (f.address || '');

        const detailsDiv = document.createElement('div');
        detailsDiv.className = 'search-item-details';

        const phoneSpan = document.createElement('span');
        phoneSpan.className = 'search-item-phone';
        phoneSpan.textContent = safeFacility ? safeFacility.phone : (f.phone || '');

        const separator = document.createTextNode(' | ');

        const typeSpan = document.createElement('span');
        typeSpan.className = 'search-item-type';
        typeSpan.textContent = [age, gender].filter(Boolean).join(' ') + (treatmentDisplay ? ` - ${treatmentDisplay}` : '');
